			...
			case -1: // Serial jam, flush received chars
				...
				switch (c) {
					...
					case -1:
						...
						if ((unsigned) (ms() - _state_ms) >= (1*_char_ms + OS_JIT_TIME)) {
							// 1 step forward...
							_state++;
							_state_ms = ms();
						}
...
DEBUG_LINE_PRINTF("/?!");
			...
			case 1: // flush sending command (delay)
				...
				switch (c) {
					...
					case -1:
						if ((unsigned) (ms() - _state_ms) >= (5*_char_ms + OS_JIT_TIME)) {
...
DEBUG_LINE_PRINTF("%c %02X (%u)", (c<'!'||c>'~'?' ':c), int(c), (ms() - _state_ms));
			...
			case 2: // check '/' 1st char of identity
				...
				switch (c) {
					...
					case -1:
						// check 1s timeout...
						if ((unsigned) (ms() - _state_ms) > 1000U) { // timeout ?
...
DEBUG_LINE_PRINTF("/ (%u)", (ms() - _state_ms));
						*_ptr++ = c;
						// step forward with timing restarted...
						_state_ms = ms();
						_state++;
...
//						_started = false;
			...
			case 3: // append char to identity str, check <CR>
				...
				switch (c) {
					...
					case -1:
						// check char timeout...
						if ((unsigned) (ms() - _state_ms) >= (1*_char_ms + OS_JIT_TIME)) { // timeout ?
DEBUG_LINE_PRINTF("<timeout>");
//							_started = false;
							_errored = true;
							// stop cycle...
...
DEBUG_LINE_PRINTF("%s", _str);
			...
			case 4: // <CR> received, check <LF>
				...
				switch (c) {
					...
					case -1:
						// check char timeout...
						if ((unsigned) (ms() - _state_ms) >= (_char_ms + OS_JIT_TIME)) { // timeout ?
DEBUG_LINE_PRINTF("<timeout>");
//							_started = false;
							_errored = true;
							// stop cycle...
...
//				if ((unsigned) (ms() - _state_ms) < 200) { // wait ?
//					// stay on step without restart timing...
//					break;
//				}
//
...
//				break;
				switch (c) {
					...
					case -1:
						if ((unsigned) (ms() - _state_ms) < 200U) { // delay ?
							// stay on step without restart timing...
						}
						else {
							// answer "protocol C"...
...
//				if ((unsigned) (ms() - _state_ms) < (6*_char_ms + OS_JIT_TIME)) break;
//
...
//				break;
				...
				switch (c) {
					...
					case -1:
						if ((unsigned) (ms() - _state_ms) < (6*_char_ms + OS_JIT_TIME)) { // delay ?
							// stay on step without restart timing...
						}
						else {
...
//				}
//
//DEBUG_LINE_PRINTF("<STX> (%u)", (ms() - _state_ms));
//
//				_bcc = 0;
				...
				switch (c) {
					...
					case -1:
						// check 2s timeout...
						if ((unsigned) (ms() - _state_ms) > 2000U) { // timeout ?
DEBUG_LINE_PRINTF("<timeout>");
//							_started = false;
							_errored = true;
							// stop cycle...
...
//	delay(1000); //workaround for above sentence
...
//BridgeClient client;
	...
	if (e230.ready(false)) { // && (client = server.accept())) {
...
#if DEBUG
debug_line_printf(__LINE__, "<ready>");
#endif
...
#if DEBUG
debug_line_printf(__LINE__, "<end>");
#endif
		}
	}
...
void loop()
{
	...
	//Serial.print('>');
			...
			if (s == String("print" STR_CRLF)) {
				//
				stream_out = & client;
			//	stream_out->print(F("\xEF\xBB\xBF")); //BOM UTF-8 https://en.wikipedia.org/wiki/Byte_order_mark
				e230.stream_out_excel();
				stream_out = & Serial;
			}
			else {
